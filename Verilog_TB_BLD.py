...
class Verilog_builder:
    ...
    def BLD(file):
        ...
        def simplify_str(set):
            modified_string = str(set)
        ...
        output_file_path = input_file_path[:-4]+'_TB_Bhaskar.v'
        ...
        with open(temp_file_path, 'w') as temp_file:
            temp_file.write(f"module {input_file_path[2:7].strip()}({simplify_str(module)});\ninput {simplify_str(ip)};\noutput {simplify_str(op)};\n\n  wire {simplify_str(wire)};\n")
